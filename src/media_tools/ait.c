...
static void gf_m2ts_process_ait(GF_M2TS_Demuxer *ts, GF_M2TS_AIT* ait){
	...
	GF_LOG(GF_LOG_INFO, GF_LOG_CONTAINER, ("[DSMCC] ChanAppInfo->nb_application %d \n",ChanAppInfo->nb_application));
	...
	for(j=0 ; j<nb_of_app ; j++){
		...
		for(k=0 ; k<nb_app_desc ; k++){
			...
			if((url_base != NULL) && ( url_appli_path != NULL)){
				...
				url_base = NULL;
				url_appli_path = NULL;
				GF_LOG(GF_LOG_INFO, GF_LOG_CONTAINER, ("[Process AIT] APPLICATION ID %d SERVICE ID %d  SERVICE NAME %s SERVICE URL %s Broadband %d \n", Application->application_id, ChanAppInfo->service_id, Application->appli_name,  Application->http_url));
			}
	...
	if(nb_of_ait){
		...
		for(i = 0;i < nb_of_ait;i++){
			...
			if(ChanAppInfo->ait_pid == pid){
				...
				GF_LOG(GF_LOG_DEBUG, GF_LOG_CONTAINER, ("[Process AIT] AIT ait_list_version number %d, ait->version_number %d \n",ChanAppInfo->version_number,version_number));
				if(ChanAppInfo->version_number >= version_number){
					GF_LOG(GF_LOG_DEBUG, GF_LOG_CONTAINER, ("[Process AIT] AIT already received, abording processing \n"));	
					return 1;
				}else{				
					...
					GF_LOG(GF_LOG_DEBUG, GF_LOG_CONTAINER, ("[Process AIT] Discard old applications - New AIT received \n"));
					return 0;
				}
			}
		}
	}else{
		GF_LOG(GF_LOG_DEBUG, GF_LOG_CONTAINER, ("[Process AIT] No AIT received \n"));
	}
